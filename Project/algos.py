...
class Solver:
	...
	def DAEM_GMM(self, X, thresh, K, mu_est=None, sigma_est=None, alpha_est=None, betas=[0.05,0.3,0.6,0.9,1.2,1.0], 
				 history_length=100, tolerance_history_thresh=1e-10, max_steps=10000):
		"""
		...
		for beta in betas:	
			...
			mu_est[0] += 1e-10*np.random.randn(n, 1) 
			mu_est[1] += 1e-10*np.random.randn(n, 1) 
			...
			while tolerance_history[-1] >= thresh and steps <= max_steps:
				...
				for k in range(K):
					...
					if np.max(tolerance_history) <= tolerance_history_thresh:
						mu_est[k] += 1e-7*np.random.randn(n, 1) 
					'''
					X_mu = X - mu_est[k]
					h_X_mu = h[k]*X_mu
					for i in range(n):
