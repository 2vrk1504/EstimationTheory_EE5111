...
class Solver:
	...
	def DAEM_GMM(self, X, thresh, K, mu_est=None, sigma_est=None, alpha_est=None, betas=[0.05,0.1, 0.2,0.5, 0.6, 0.9, 1.2,1.1,1.05,1.0], 
			...
			while tolerance_history[-1] >= thresh and steps <= max_steps:
				...
				for k in range(K):
					...
					if np.max(tolerance_history) <= tolerance_history_thresh:
						mu_est[k] += 1e-10*np.random.randn(n, 1) 
